...
class Dataset(data.Dataset):
    def __init__(
        ...
        if modality_folder_names is not None:
            ...
            if n0 == 0:
                raise ValueError('membrane+/nuclear+ データセット: 無染色6入力の画像が0枚です。')
        ...
        if modality_folder_names is not None and (blend != 'concatenate' or len(use_list) != 6):
            raise Exception('membrane+/nuclear+ 無染色6入力では blend=concatenate かつ len(use_list)==6 が必要です。')
    ...
    def __getitem__(self, index):
        if self.blend == 'concatenate':
            ...
            if len(self.use_list)==3:#撮像法のみの検討
                ...
                if self.mask == "membrane+":
                    blocks.append(cv2.cvtColor(cv2.imread(self.y_nuclear_img_paths[index], cv2.IMREAD_GRAYSCALE), cv2.COLOR_GRAY2RGB))
                elif self.mask == "nuclear+":
                    blocks.append(cv2.cvtColor(cv2.imread(self.y_membrane_img_paths[index], cv2.IMREAD_GRAYSCALE), cv2.COLOR_GRAY2RGB))
                else:
                    raise Exception(f'mask が不正です: {self.mask}')
...
def get_dataloader(folder_path, use_list, mask, color='RGB', blend='concatenate', other_channel=False, batch_size = 1, num_workers=0, isShuffle=True, pin_memory=True, modality_folder_names: Optional[Tuple[str, ...]] = None, use_rgb_balance: bool = False, use_rgb_chromatic: bool = False, use_rgb_core: bool = True):
    dataset = Dataset(
        folder_path, use_list, mask, color=color, blend=blend, other_channel=other_channel,
...
def get_image(img_path_list, use_list, color='RGB', blend='concatenate', use_rgb_balance: bool = False, use_rgb_chromatic: bool = False, use_rgb_core: bool = True):
    if blend == 'concatenate':
        ...
        if len(use_list)==3:#撮像法のみの検討
            ...
            if len(img_path_list) < 7:
                raise Exception(f'無染色6入力の membrane+/nuclear+ では img_path_list に6画像+補助1の7経路が必要です: len={len(img_path_list)}')
            ...
            imgs.append(_get_image(img_path_list[6], color))
